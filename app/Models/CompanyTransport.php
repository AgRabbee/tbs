...
class CompanyTransport extends Model
{
    ...
    public function busDetails()
    {
        $details = DB::table('company_transport')
            ->selectRaw('company_transport.*,t.ac_type as ac_type, tp.*, tp.id as trip_id, s.name as start_name, e.name as end_name')
            ->join('transports as t','company_transport.transport_id','t.id')
            ->join('trips as tp','company_transport.transport_id','tp.bus_id')
